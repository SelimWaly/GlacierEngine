...
using namespace std;
//	, java
#define DEFAULT_DATA_PATH "/sdcard/glacier/"
...
int StartGlacierEngine(const char* szAppDataPath, const char* savPath, unsigned char* pJavaSurfaceBuf,
...
JNIEXPORT void JNICALL Java_com_Core_drawIntoBitmap(JNIEnv* env,
  ...
  (JNIEnv * env, jclass thiz, jstring jstrPath, jstring jsavPath, jobject dstBitmap, jint iDeviceWidth, jint iDeviceHeight)
{
	...
	LOGI ("DataPath = %s, jsavPath = %s, dstPixels = %p, rect(%d, %d, %d), device (%d, %d)",
	 ...
	 if(!IsInit)
	 {
		 ...
		 StartGlacierEngine(strPathName.c_str(), savPathName.c_str(), (unsigned char*)dstPixels, dstInfo.width, dstInfo.height, iDeviceWidth,  iDeviceHeight);
  ...
  (JNIEnv * env, jclass thiz, jint message, jint wParam, jint lParam, jint iBufSize, jbyteArray pJavaBytes)
{
	...
	if (array == NULL)
	{
		LOGE ("Java_com_Core_sendGLACIERMessageBuf GetByteArrayElements error\n");
		return;
	}
	//	wParam - pointcount
...
/*
 * Class:     com_koteinavi_GlacierCore
