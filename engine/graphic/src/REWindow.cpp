...
REDDrawSurface::REDDrawSurface(HWND hwnd,kn_int w, kn_int h, REBitmap::Config cf)
{
	...
	if (FAILED(DirectDrawCreate(NULL,&m_pMyDD,NULL)))goto error ;//
	if (FAILED(m_pMyDD->SetCooperativeLevel(m_hwnd,DDSCL_NORMAL)))goto error ;//

//	if (FAILED(m_pMyDD->SetCooperativeLevel(m_hwnd, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN)))goto error ;//
...
{
	//if (m_gl_env->m_b_init == FALSE)
	//{
	//	m_gl_env->glenvInit(m_hdc, 0,0, GetWidth(), GetHeight() );
	//	
	//}
	m_gl_env->glenvActive();
...
REGLESSurface::REGLESSurface(kn_int w, kn_int h, REBitmap::Config cf)
{
	...
	//extern KGlesEnv g_gles_env;
	//setGLESEnv(&g_gles_env);
