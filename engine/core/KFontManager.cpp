...
#define ChineseSampleText _T("Windows  1234567890")
...
{ 
	...
	if (bFoundMaxpTable)
	{
		...
		fontProp.strGlyphCount = _T(" ")+fontProp.strGlyphCount+_T(" ");
	}
	//find "hhea" table
	if (bFoundHheaTable)
	{
		...
		for(int i = 0; i < nameTable.uRecordCount; i++)
		{
			...
			if (ttRecord.uPlatformID==0x03)
			{
				...
				else if(ttRecord.uNameID == 1&&ttRecord.uLanguageID == 0x0804)
				{
					...
					kn_string::size_type pos = fontProp.strLanguage.find(_T(""));
					if (pos==kn_string::npos)
					{
						fontProp.strLanguage.append(_T(" "));
					}
				}
				//chinese copyright
...
RETypeface* KFontManager::GetFontFromName( char* szName, SkTypeface::Style s )
{
	...
	{//m_FontBoldNameMapm_FontNameMap
