...
void KSvgAnalyzing::LoadSvgFile(kn_string& filePath,vector<KShape* >& pShapeList,RERect& rect,bool bScaleFlag)
{
	...
	TiXmlElement* rootElement = myDocument->RootElement();  //svg 
	...
	TiXmlElement* nodeElement = rootElement->FirstChildElement(); //pathrect...
	KShape * pShape(NULL);
	while ( nodeElement ) {
		pShape = NULL;
...
{
	char delim[] = "MmLlHhVvCcSsQqTtAaZz"; //
	vector<string>sectionList;
	int len = strlen(pPoints);
	char tempBuff[128]={0};
	int recordIndex = 0; //index
...
void KSvgAnalyzing::AnalyzingPath(vector<string>& sectionList,REPath& path)
{
	...
	{	
		...
		char* pChar = tempBuff + 1; //
		char type = tempBuff[0];   //
		PathData pathData;
		switch (type)
		{
		case 'M':
		...
		{	
			...
			if (*(pChar+i) == '\0' || strrchr(delim,*(pChar+i)))
			{
				//“-”
				float data;
				if(dataBuff[0] == ','&&strlen(dataBuff) > 1)  //",x"
				{
				...
				else 
				{
					...
					else if (dataBuff[0] > 47&&dataBuff[0] < 58)//ASCII  48 ~ 57
					{
						data = atof(dataBuff);
						pathData.PathUnitDataList.push_back(data);
					}
