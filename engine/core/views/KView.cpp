...
kn_bool KView::DoMessage(KMessage* pMsg)
{
	...
	{//view m_b_mouse_picked m_b_mouse_in_view view
...
// 	}
	...
	case KMSG_TOUCH_DOWN:
		{
			...
			//ret = TRUE;
			onTouchDownDirect(pTouchMsg->m_pos_x, pTouchMsg->m_pos_y, pTouchMsg); // 
	...
	case KMSG_TOUCH_MOVE:
		{
			...
			{
				// 1 TOUCH_DOWM 1point
				// 2. No TOUCH_DOWM 2point [TOUCH_DOWM 2point ] 
				// 3 TOUCH_MOVE 2points,   
				handleMouseUp(pMsg);	
	...
	case KMSG_TOUCH_UP:
		{
			KMessageTouch* pTouchMsg = (KMessageTouch*)pMsg;
			if(pTouchMsg->m_iPointCount == 1 && m_dw_touctpoint_count == 1)	//	1
			{
				//  KMSG_LBBUTTONUP 			 
				handleMouseUp(pMsg);
	...
	default:
		ret = false;
		if (pMsg->m_msg_type > KGLACIERMSG_USER)
		{//
			ret = OnUserMsg(pMsg);
		}
		break;
